...
<div class="mt-20 py-6 bg-gray-100">
    <h1 class="text-4xl font-bold text-center animate-fade-in-down">
        <span class="bg-gradient-to-r from-blue-600 to-indigo-600 text-transparent bg-clip-text">
            ✨ Edit Trip: {{ $tourtrip->name }}
        </span>
    </h1>
</div>
<div class="container mx-auto shadow-4xl bg-gray-100">
    <form action="{{ route('tourtripsupdate', $tourtrip->id) }}" method="POST" class="mx-auto max-w-4xl bg-white p-8 rounded-xl shadow-4xl shadow-blue-100/50 relative transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-100/70" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        
        <a href="{{ route('tourshow',$tourtrip->id) }}" class="absolute top-2 right-2">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-4 rounded-xl overflow-hidden border border-gray-100 shadow-sm bg-gray-50 p-4">
                    <img src="{{ asset('images/tourtrips/' . $tourtrip->image) }}" alt="Current image" class="w-full object-contain max-h-64 mx-auto">
                </div>
                @endif
            </div>
