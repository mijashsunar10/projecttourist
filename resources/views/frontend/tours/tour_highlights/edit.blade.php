...
<div class="max-w-4xl mx-auto mt-20 bg-white shadow-lg rounded-lg p-6 md:p-8 relative">
    ...
    <a href="{{ route('tourtripshow', $tourtrip->id) }}" class="absolute top-4 right-4 text-gray-500 hover:text-red-600 transition text-2xl">
        ❌
    </a>

    <h1 class="text-2xl md:text-3xl font-bold text-blue-700 text-center mb-6">✨ Edit Highlights for "{{ $tourtrip->name }}"</h1>
    ...
    <form action="{{ route('tourHighlightsupdate', $tourtrip->id) }}" method="POST">
        @csrf
        <div id="highlight-container" class="space-y-4">
            @foreach ($highlights as $highlight)
                <div class="flex items-center border border-gray-300 rounded-lg p-3 bg-gray-50 shadow-sm relative">
                    <textarea name="tourhighlights[{{ $highlight->id }}]" class="w-full border-none bg-transparent focus:outline-none text-gray-700 resize-none p-2" rows="2">{{ $highlight->highlight }}</textarea>
...
<script>
    document.getElementById('add-highlight').addEventListener('click', () => {
        ...
        newTextarea.name = 'tourhighlights[new][]';
