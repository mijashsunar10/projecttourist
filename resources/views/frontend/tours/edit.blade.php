...
<div class="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen p-6 flex justify-center items-center">
    <div class="max-w-2xl w-full bg-white p-8 rounded-xl shadow-2xl shadow-blue-100/50 relative transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-100/70 mt-20">
        <div class="text-center mb-6 animate-fade-in-down">
            ...
            <p class="text-lg text-gray-600">Refine and update your tour details</p>
        </div>
        
        <!-- Cancel Button -->
        <a href="{{ route('tourindex') }}" class="absolute top-2 right-2">
        ...
        <form action="{{ route('tourupdate', $tour->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{-- @method('PUT') --}}
            <div class="space-y-6">
                <!-- Name Input -->
                <div>
                    ...
                    <input type="text" name="name" id="name" 
                           ...
                           value="{{ $tour->name }}" placeholder="Enter tour name" required>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div class="rounded-xl overflow-hidden border border-gray-100 shadow-sm bg-gray-50 p-4">
                        <img src="{{ asset('images/tours/' . $tour->image) }}" 
                             alt="Current tour image" class="w-full object-contain max-h-64 mx-auto">
                    </div>
                </div>
                @endif
