...
<div class="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen p-6 flex justify-center items-center">
    <div class="max-w-2xl w-full bg-white p-8 rounded-xl shadow-2xl shadow-blue-100/50 relative transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-100/70 mt-20">
        <div class="text-center mb-6 animate-fade-in-down">
            ...
            <p class="text-lg text-gray-600">Refine and update your expedition details</p>
        </div>
        
        <!-- Cancel Button -->
        <a href="{{ route('expeditionsindex') }}" class="absolute top-2 right-2">
        ...
        <form action="{{ route('expeditionsupdate', $expedition->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{-- @method('PUT') --}}
            <div class="space-y-6">
                <!-- Name Input -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">expedition Name</label>
                    <input type="text" name="name" id="name" 
                           ...
                           value="{{ $expedition->name }}" placeholder="Enter expedition name" required>
                </div>
                
                <!-- Existing expedition Image Preview -->
                @if ($expedition->image)
                <div class="mt-4">
                    ...
                    <div class="rounded-xl overflow-hidden border border-gray-100 shadow-sm bg-gray-50 p-4">
                        <img src="{{ asset('images/expeditions/' . $expedition->image) }}" 
                             alt="Current expedition image" class="w-full object-contain max-h-64 mx-auto">
                    </div>
                </div>
                @endif
                        ...
                        <div class="relative group">
                            <button type="button" id="remove-preview" 
                                    class="absolute -top-3 -right-3 bg-red-500 text-white rounded-full w-7 h-7 
                                           ...
                                           shadow-sm hover:shadow-md">&times;</button>
