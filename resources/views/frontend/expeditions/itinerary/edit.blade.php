...
<div class="max-w-4xl mx-auto mt-20 bg-white shadow-lg rounded-lg p-6 md:p-8 relative">
    ...
    <h1 class="text-3xl font-bold text-blue-700 text-center mb-6">📝 Edit Itinerary for "{{ $mountain->name }}"</h1>

    <form action="{{ route('mountainitineraryupdate', [$mountain->id, $itinerary->id]) }}" method="POST" class="space-y-6">
        @csrf
