...
<div class="container mx-auto py-8 px-16 bg-white shadow-md rounded-lg mt-8">
    <h2 class="text-3xl font-bold text-blue-700 mb-6">Edit mountain Fact for {{ $mountain->name }}</h2>
    <form action="{{ route('mountainfactupdate', [$mountain->id, $fact->id]) }}" method="POST">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
                <label class="text-sm text-gray-600">mountain Start and End</label>
            ...
            <div>
                <label class="text-sm text-gray-600">mountain Area</label>
        ...
        <div class="mt-6">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Update mountain Fact</button>
            <a href="{{ route('mountainshow', $mountain->id) }}" class="ml-4 text-blue-600">Cancel</a>
        </div>
    </form>
</div>
@endsection
