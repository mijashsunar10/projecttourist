            ...
            <div class="w-full text-center h-full relative ">
                ...
                <div class="absolute top-3 z-10 mt-4 right-4 flex gap-2">
                    ...
                    <div id="addImageModal" class="fixed inset-0 z-10 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center">
                        ...
                        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg relative">
                            ...
                            <form action="{{ route('addtripimages', $trip->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                {{-- Display errors for individual images --}}
                                @error('images.*')
                                    <p class="text-red-500 text-md mt-1">{{ $message }}</p>
                                @enderror
                                </div>
                    ...
                    @if ($trip->images->isNotEmpty())
                        <form action="{{ route('tripdeleteimage', $trip->images->first()->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 text-sm font-semibold rounded-lg shadow">
                                Delete
                            </button>
                        </form>
                    @endif
                ...
                <!-- Thumbnail Images -->
                @if ($trip->images->isNotEmpty())
                    <div class="flex flex-wrap gap-4 justify-center absolute bottom-8 w-full z-1 px-4">
                        @foreach ($trip->images as $image)
                            <img src="{{ asset('images/trips/' . $image->image) }}" 
                                ...
                                {{-- class="h-16 w-24 sm:h-20 sm:w-32 md:h-24 md:w-36 object-cover rounded-lg cursor-pointer small-image" /> --}}
...
<div class="px-8 py-12 flex flex-col lg:flex-row gap-6 z-40">
    ...
    <!-- Left Section: Trip Details (80% on large screens) -->
    <div class="w-full lg:w-4/5 mx-auto">
        ...
        <div class="flex flex-wrap items-center justify-center w-full max-w-4xl mx-auto xs:mb-4 mt-8">
            ...
            <div id="tripfacts" class="py-8 px-4 sm:px-8 bg-white shadow-md rounded-lg mt-8 ">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="mt-2">
                        ...
                        <form action="{{ route('tripfactdestroy', [$trip->id, $fact->id]) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
            ...
            @auth
                <div class="mx-auto mt-8 p-12 bg-white rounded-lg shadow-xl" id="highlight" >
                    ...
                    <div style="max-width: 98%" class="mx-auto">
                        <h2 class="text-2xl sm:text-3xl font-bold mb-4 text-blue-900 " style=" font-family: 'Times New Roman', Times, serif">Trip Highlights</h2>
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row space-y-4 sm:space-x-4 sm:space-y-0">
                        ...
                        <a href="{{ route('tripHighlightsedit', $trip->id) }}" class="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 transition-colors duration-300">
                            Edit Highlights
                        </a>
                        <a href="{{ route('tripHighlightscreate', $trip->id) }}" class="bg-green-500 text-white px-6 py-2 rounded-md hover:bg-green-600 transition-colors duration-300">
                            Add Highlights
                        </a>
                    </div>
                </div>
                        ...
                        <div class="w-full">
                            <div class="bg-blue-900 text-white text-center py-6 rounded-t-lg">
                                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold" 
                                    style="font-family: 'Times New Roman', Times, serif">
                                    Trekking in Nepal – Itinerary Overview
                                </h1>
                                <p class="mt-2 text-sm sm:text-base">A detailed description of the itinerary</p>
            
                                @auth
                                    <a href="{{ route('itinerarycreate', $trip->id) }}" class="inline-block mt-3">
                                        <button class="text-white font-bold px-4 py-2 bg-gray-700 rounded-lg hover:bg-gray-800">
                                            Add Itinerary
                                        </button>
                                    </a>
                                @endauth
                            ...
                            <div id="faq-container" class="bg-white shadow-lg rounded-b-lg p-4">
                                @foreach ($itineraries as $itinerary)
                                    <div class="border-b mb-4 last:mb-0">
                                        <button class="w-full flex justify-between items-center text-left p-4 text-base sm:text-lg font-semibold text-blue-900 bg-white hover:bg-gray-100 shadow-lg focus:outline-none transition duration-200"
                                            ...
                                            {{ $itinerary->question }}
                                            <svg id="icon{{ $itinerary->id }}" class="ml-2 w-5 h-5 text-gray-800 transition-transform transform rotate-0"
                                            ...
                                            @auth
                                                <div class="mt-3 flex flex-wrap gap-2">
                                                    <a href="{{ route('itineraryedit', [$trip->id, $itinerary->id]) }}"  class="text-blue-500">
                                                        <button class="text-white font-bold px-3 py-1 bg-blue-600 rounded-lg hover:bg-blue-700">
                                                            Edit
                                                        </button>
                                                    </a>
                                                    <form action="{{ route('itinerarydestroy', $itinerary->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-white font-bold px-3 py-1 bg-red-500 rounded-lg ">
                                                            Delete
                                                        </button>
                                                    </form>
                                                </div>
                    ...
                    <div class="w-full">
                        ...
                        @foreach($trip->requiredItems as $item)
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b py-3">
                                <p class="text-gray-700 text-md sm:text-lg font-semibold flex items-center">
                                    &#10148; <span class="px-3">{{ $item->item_name }}</span>
                                </p>
                                @auth
                                    <div class="flex flex-wrap gap-2 mt-2 sm:mt-0">
                                        <a href="{{ route('requireditems.edit', [$trip->id, $item->id]) }}" 
                                           class="bg-blue-500 text-white px-3 py-1 rounded text-sm sm:text-base hover:bg-blue-600">
                                            Edit
                                        </a>
                                        <form action="{{ route('requireditems.destroy', [$trip->id, $item->id]) }}" method="POST">
                                            ...
                                            @method('DELETE')
                                            <button type="submit" 
                                                    class="bg-red-500 text-white px-3 py-1 rounded text-sm sm:text-base hover:bg-red-600">
                                                Delete
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @auth
                            <div class="">
                                <a href="{{ route('requireditems.create', $trip->id) }}" 
                                   class="mt-6 inline-block bg-green-500 text-white px-5 py-2 rounded text-sm sm:text-base hover:bg-green-600">
                                    Add Required Item
                                </a>
                            </div>
                        @endauth
            ...
            <div class="container mx-auto py-12 px-6 sm:px-12 bg-gray-100 mt-8" id="inclusions">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6 text-blue-900 "
                    style="font-family: 'Times New Roman', Times, serif">
                ...
                @auth
                    <div class="mt-6 text-center">
                        <a href="{{ route('trips.inclusions-exclusions.create', $trip->id) }}" 
                           class="bg-blue-500 text-white px-6 py-2 rounded-md shadow-md w-full sm:w-auto inline-block hover:bg-blue-600 transition duration-300">
                            Add Items
                        </a>
                    </div>
                @endauth
    ...
    <div class="mt-6">
        <section class="  bg-gray-100 mt-8 p-6 md:p-9" id="faqs">
            <div class="mx-auto ">
                <h2 class=" font-bold mb-4 text-blue-900  text-2xl md:text-3xl" style="font-family: 'Times New Roman', Times, serif;">
                    {{$trip->name}} FAQ Overview
                </h2>
                <div class="flex justify-center md:justify-start">
                    <div class="w-full">
                        <div class="bg-blue-900 text-white text-center p-6 rounded-t-lg mb-2 sm:mb-4">
                            <h1 class="text-2xl md:text-3xl font-bold" style="font-family: 'Times New Roman', Times, serif;">FAQ Overview</h1>
                            <p class="mt-2 text-lg">A detailed description of Trip FAQs</p>
                            @auth
                            ...
                            @foreach ($tripfaqs as $tripfaq)
                                <div class="border-2 mb-2 sm:mb-4 border-gray-100 ">
                                    <button
                                        ...
                                        onclick="toggleFAQAnswer('answer{{ $tripfaq->id }}')" aria-expanded="false">
                                        {{ $tripfaq->question }}
                                    ...
                                    <div class="hidden px-4 pb-4 bg-white text-black" id="answer{{ $tripfaq->id }}">
                                        <p class="text-sm md:text-base">{{ $tripfaq->answer }}</p>
    ...
    <div class="bg-gray-50 p-10 mt-6" id="reviews">
        <div class="">
            ...
            <div class="mt-8 text-center mb-2">
                <a href="{{ route('reviews.index', $trip_id) }}" class="inline-block px-6 py-2 bg-blue-800 text-white rounded-md hover:bg-blue-600 transition duration-200 ease-in-out mb-2">
                    View All Reviews
                </a>
    ...
    <div id="reviewFormContainer" class="container py-8 px-4 bg-gray-100 rounded-lg shadow-lg mt-10" style="display: none;">
        ...
        <form action="{{ route('reviews.store', $trip->id) }}" method="POST" enctype="multipart/form-data" class="bg-white mx-auto p-8 rounded-lg shadow-md" style="max-width:95%">
            ...
            <div class="mt-6 text-center">
                <button type="submit" class="px-8 py-3 bg-blue-800 text-white rounded-lg hover:bg-blue-600 transition duration-200 ease-in-out">
                    Submit Review
                </button>
            </div>
        </form>
