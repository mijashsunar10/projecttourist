...
<div class="container mx-auto py-8 px-16 bg-white shadow-md rounded-lg mt-8">
    <h2 class="text-3xl font-bold text-blue-700 mb-6">Edit Trip Fact for {{ $trip->name }}</h2>
    <form action="{{ route('tripfactupdate', [$trip->id, $fact->id]) }}" method="POST">
        @csrf
        ...
        <div class="mt-6">
            ...
            <a href="{{ route('tripshow', $trip->id) }}" class="ml-4 text-blue-600">Cancel</a>
        </div>
    </form>
</div>
@endsection
