    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center mb-8">
            <div class="sm:flex-auto">
                <h1 class="text-2xl font-bold text-gray-900">Enquiry  Messages</h1>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($enquiries as $enquiry)
                            <tr class="hover:bg-gray-50 transition-colors duration-150 cursor-pointer group" onclick="window.location='{{ route('admin.enquiry.show', $enquiry->id) }}'">
                                <td class="pl-6 pr-3 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm font-semibold text-gray-900">{{ $enquiry->name }}</div>
                                            <div class="text-xs text-gray-500 mt-1">{{ $enquiry->created_at->format('M d, Y h:i A') }}</div>
                                        </div>
                                    </div>
                                </td>
                ...
                <div class="border-t border-gray-100 px-6 py-4">
                    <nav class="flex items-center justify-between">
                        <div class="text-sm text-gray-500">
                            Showing {{ $booking->firstItem() }} - {{ $booking->lastItem() }} of {{ $booking->total() }}
                        </div>
                        <div class="flex space-x-2">
                            @foreach ($booking->links()->elements as $element)
                                @if(is_array($element))
