    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center mb-8">
            <div class="sm:flex-auto">
                <h1 class="text-2xl font-bold text-gray-900">Customize Trip Messages</h1>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($customizes as $customize)
                            <tr class="hover:bg-gray-50 transition-colors duration-150 cursor-pointer group" onclick="window.location='{{ route('admin.customizes.show', $customize->id) }}'">
                                <td class="pl-6 pr-3 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm font-semibold text-gray-900">{{ $customize->name }}</div>
                                            <div class="text-xs text-gray-500 mt-1">{{ $customize->created_at->format('M d, Y h:i A') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $customize->email }}</div>
                                </td>
                                <td class="px-3 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                ...
                                <td class="px-3 py-4 whitespace-nowrap">
                                    <form action="{{ route('admin.customizes.destroy', $customize->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
            ...
            @if($customizes->hasPages())
                <div class="border-t border-gray-100 px-6 py-4">
                    <nav class="flex items-center justify-between">
                        <div class="text-sm text-gray-500">
                            Showing {{ $customize->firstItem() }} - {{ $customize->lastItem() }} of {{ $customize->total() }}
                        </div>
                        <div class="flex space-x-2">
                            @foreach ($customize->links()->elements as $element)
                                @if(is_array($element))
                                    @foreach ($element as $page => $url)
                                        @if($page == $customize->currentPage())
