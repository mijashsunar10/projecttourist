...
@section('pagecontent')
<div class="container mx-auto mt-20 p-6 bg-white shadow-lg rounded-xl max-w-2xl">
    <!-- Header Section -->
    <div class="text-center mb-6 animate-fade-in-down">
        ...
        <p class="text-gray-600">Modify the document details</p>
    </div>
    ...
    <form action="{{ route('documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="category" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none" required>
                    <option value="legal_documents" {{ $document->category == 'legal_documents' ? 'selected' : '' }}>Legal Documents</option>
                    <option value="travel_association" {{ $document->category == 'travel_association' ? 'selected' : '' }}>Travel Associations</option>
                    <option value="awards" {{ $document->category == 'awards' ? 'selected' : '' }}>Awards</option>
                </select>
            </div>
                ...
                <div id="preview-container" class="mt-6">
                    <div class="relative group">
                        <button type="button" id="remove-preview"
                            class="absolute -top-3 -right-3 bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center hover:bg-red-600 transition-all shadow-sm hover:shadow-md hidden">&times;</button>
                        <div class="rounded-xl overflow-hidden border border-gray-100 shadow-sm">
                            <img id="image-preview" class="w-full object-contain max-h-96 bg-gray-50"
                                src="{{ asset('images/documents/' . $document->image) }}" alt="Current Image">
                        </div>
                    </div>
                </div>
            </div>
